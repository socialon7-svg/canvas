...
begin
  if to_regclass('public.lean_canvas_submissions') is not null
    and not exists (
      select 1
      from pg_constraint
      where conname = 'lean_canvas_submissions_pdf_status_check'
        and conrelid = 'public.lean_canvas_submissions'::regclass
    )
  then
  ...
  if to_regclass('public.module_submissions') is not null
    and not exists (
      select 1
      from pg_constraint
      where conname = 'module_submissions_pdf_status_check'
        and conrelid = 'public.module_submissions'::regclass
    )
  then
    alter table public.module_submissions
      add constraint module_submissions_pdf_status_check
      check (pdf_status in ('idle', 'generating', 'success', 'failed'));
  end if;
end $$;
