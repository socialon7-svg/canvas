...
import { z } from "zod";
import { generateCustomerInterviewDraft } from "@/lib/ai";
import { authorizeActiveOperationRequest } from "@/lib/participantAuth";
import type { CustomerInterviewInput } from "@/lib/types";
...
const customerInterviewInputSchema = z.object({
  programName: textField(300),
  teamName: textField(200),
  participantName: textField(100),
  ideaMemo: z.string().trim().min(1, "인터뷰 질문지를 만들 고객 문제 메모를 입력해주세요.").max(3000),
...
export async function POST(request: Request) {
  let input: CustomerInterviewInput;

  try {
    input = customerInterviewInputSchema.parse(await request.json()) as CustomerInterviewInput;
  } catch (error) {
  ...
  try {
    ...
    const draft = await generateCustomerInterviewDraft(input);
