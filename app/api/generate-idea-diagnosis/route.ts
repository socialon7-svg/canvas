...
const ideaDiagnosisInputSchema = z.object({
  programName: textField(300),
  teamName: textField(200),
  participantName: textField(100),
  ideaMemo: z.string().trim().min(1, "진단할 아이디어 내용을 입력해주세요.").max(3000),
  oneLineIdea: textField(1000),
  operation: operationContextSchema
});

export async function POST(request: Request) {
  let input: IdeaDiagnosisInput;

  try {
    input = ideaDiagnosisInputSchema.parse(await request.json()) as IdeaDiagnosisInput;
  } catch (error) {
