...
async function findModuStartupSubmission(id: string) {
  const row = await getModuleSubmission(id);
  return row?.module_slug === "modu-startup-application" ? row : null;
}

export async function GET(request: Request, { params }: { params: Promise<unknown> }) {
  try {
    const { id } = (await params) as { id: string };
    const row = await findModuStartupSubmission(id);
    ...
    return NextResponse.json({ submission: toModuStartupSubmission(row) });
  } catch (error) {
    return handleOperationsApiError(error, "모두의창업 제출물 조회 실패");
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<unknown> }) {
  try {
    const { id } = (await params) as { id: string };
    const existing = await findModuStartupSubmission(id);
    ...
    return handleOperationsApiError(error, "모두의창업 PDF 상태 저장 실패");
  }
}
