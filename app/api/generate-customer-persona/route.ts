...
import { z } from "zod";
import { generateCustomerPersonaDraft } from "@/lib/ai";
import type { CustomerPersonaInput } from "@/lib/types";
...
const customerPersonaInputSchema = z.object({
  programName: textField(300),
  teamName: textField(200),
  participantName: textField(100),
  ideaMemo: z.string().trim().min(1, "페르소나를 만들 아이디어 내용을 입력해주세요.").max(3000),
...
export async function POST(request: Request) {
  let input: CustomerPersonaInput;

  try {
    input = customerPersonaInputSchema.parse(await request.json()) as CustomerPersonaInput;
  } catch (error) {
  ...
  try {
    const draft = await generateCustomerPersonaDraft(input);
