...
import { z } from "zod";
import { generateCustomerSurveyDraft } from "@/lib/ai";
import { authorizeActiveOperationRequest } from "@/lib/participantAuth";
import type { CustomerSurveyInput } from "@/lib/types";
...
const customerSurveyInputSchema = z.object({
  programName: textField(300),
  teamName: textField(200),
  participantName: textField(100),
  ideaMemo: z.string().trim().min(1, "설문지를 만들 고객 문제 메모를 입력해주세요.").max(3000),
...
export async function POST(request: Request) {
  let input: CustomerSurveyInput;

  try {
    input = customerSurveyInputSchema.parse(await request.json()) as CustomerSurveyInput;
  } catch (error) {
  ...
  try {
    ...
    const draft = await generateCustomerSurveyDraft(input);
