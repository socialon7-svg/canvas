...
- 작성 중 네트워크가 끊기면 브라우저 초안을 유지하고, 연결 복구 시 서버에 자동 재동기화합니다. 서버 초안과 브라우저 초안이 모두 있으면 저장 시각이 더 최신인 내용을 복구합니다.
...
| `/internal?tab=moduStartup` | 운영 콘솔의 모두의창업 제출 목록 |
| `/admin/modu-startup` | 레거시 주소이며 운영 콘솔 모두의창업 탭으로 이동 |
| `/internal` | 내부 운영 포털 |
...
프로그램/참여자 관리 API는 관리자 httpOnly cookie 인증을 사용합니다. 참여자 입장 API는 프로그램 코드+참여자 코드 또는 `join_token`으로 참여자를 확인하고, 정상 입장 시 HMAC 서명된 httpOnly 참여자 세션을 발급하며 `joined_at`, `last_seen_at`을 갱신합니다. `module-drafts`, `module-progress`, 참여자 제출 API와 모든 AI 생성 API는 세션의 `programId`, `participantId`와 요청값이 다르면 403을 반환합니다. AI 생성은 유효한 참여자 세션 또는 관리자 세션이 있어야 호출할 수 있어 외부의 무단 API 비용 소모를 차단합니다. Supabase 운영 테이블이 준비되지 않은 경우에만 기존 localStorage 데모 흐름으로 fallback합니다.
...
- 1~13번 특화 모듈은 결과 형식은 유지하되 공통 클라이언트 Runner가 API 요청·오류·저장을 처리함
- 15번 이후 구조화 모듈은 `moduleRunner.ts`의 공통 입력·출력 스키마와 생성 API를 사용하며, 린캔버스와 모두의창업은 전용 편집 화면을 유지함
- `npm run check:modules`는 26개 카탈로그와 특화 Runner·공통 Runner·전용 화면·API route 연결을 검증하며 프로덕션 빌드 전에 자동 실행됨
